<?php
  require '../Components/header.php';
  require '../Components/navbar.php';
  require '../Db/database.php';
  Include '../function/film.php';
  Include '../function/actor.php';

?>


<div class="row">
    <?php $films = Film::all();
    foreach ($films as $data) {
    ?>
    
        <div class="card m-3 mx-auto text-center" style="width: 18rem;">
            <h5 class="card-title text-center"><strong><?= $data['title']; ?></strong></h5>
            <div class="col-md-12 d-flex text-center">
                <h2 class="col-md-6"><?= $data['rental_rate']; ?>  
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-half" viewBox="0 0 16 16">
                    <path d="M5.354 5.119 7.538.792A.516.516 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.537.537 0 0 1 16 6.32a.548.548 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.52.52 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.58.58 0 0 1 .085-.302.513.513 0 0 1 .37-.245l4.898-.696zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.565.565 0 0 1 .162-.505l2.907-2.77-4.052-.576a.525.525 0 0 1-.393-.288L8.001 2.223 8 2.226v9.8z"/>
                    </svg>
                </h2>
                <h3 class="col-md-6"><?= $data['name']; ?></h3>
            </div>
            <div class="card-body overflow-hidden h-20 text-center">
                <p class="card-text">Release year  : <?= $data['release_year']; ?></p>
                <?php
                $filmId = (int)$data['film_id'];
                $actors = Actor::readByFilm($filmId);
                ?>
                <p class="text-decoration-underline">
                    Actors : <br>
                </p>
                <?php foreach ($actors as $data) { ?>
                    <small><?= $data['first_name'] . " " . $data['last_name'] ?>
                        <br>
                    </small>
                <?php } ?>
                <div class="btn col-md-12 ">
                    <a href="movie.php?id=<?= $filmId ?>" class="btn btn-info">SEE MORE</a>
                </div>
            </div>
        </div>
    <?php } ?>
</div>
