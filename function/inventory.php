...
class Inventory extends Database
{
    ...
    {
        $inventories = self::query(' SELECT *FROM inventory');
    ...
    {
        $inventory = self::query("SELECT *  FROM inventory WHERE film_id=$id");
        return $inventory->fetchall();
    }

    public static function readByStore($id)
    {
        $inventory = self::query("SELECT *, COUNT(film_id) nbr FROM inventory 
                                    WHERE store_id=$id 
                                    GROUP BY film_id ");
        return $inventory->fetchall();
    }
